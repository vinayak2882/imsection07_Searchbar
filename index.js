const Data = [
  ...
  var timedelay ;
  function hudaklavkar(pablo,delaytime){
    setTimeout(function(){
        if(timedelay){
  ...
  function pablo(){
    ...
    let Contrydata = Data
    .filter((el) =>
      el.countryName.toLowerCase().indexOf(SearchData) !== -1 ? true : false
    )
    .map((items) => items.countryName);
  
  finalAppend(Contrydata);
  ...
  function finalAppend(data) {
    document.getElementById("outputdiv").textContent = null;
    ...
    data.map((element) => {
      ...
      div.append(p);
      document.getElementById("outputdiv").append(div);
    });
  }